...
def getFlowCommon(flow, btag):
    ...
    flow.Define("nMuon_looseId", "nMuon_iso + Sum(Muon_looseId)")
    flow.Define("nMuon_pt", "nMuon_looseId + Sum(Muon_pt > 20.)")
    flow.Define("nMuon_eta", "nMuon_pt + Sum(abs(Muon_eta) < 2.4)")
    flow.Define("nMuon_dxy", "nMuon_eta + Sum(Muon_dxy < 0.5)")
    flow.Define("nMuon_dz", "nMuon_dxy + Sum(Muon_dz < 0.1)")
