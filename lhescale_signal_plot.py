...
lumi = 59970
sample = "ZH"
xsec = 0.05162528008  # ZH
# sample = "ggZH"
# xsec = 0.0072158061930000005  # ggZH

for histo in histo_list:
    f1 = ROOT.TFile.Open(
        f"/home/filippo/Downloads/SignalLHEScaleNom_deepflav_NOsnap_eval/{sample}_Histos.root"
    )
    full_nom = f1.Get(histo + "___SR_ee")
    f2 = ROOT.TFile.Open(
        f"/home/filippo/Downloads/SignalLHEScale1_deepflav_NOsnap_eval/{sample}_Histos.root"
    )
    full_up = f2.Get(histo + "___SR_ee")
    f3 = ROOT.TFile.Open(
        f"/home/filippo/Downloads/SignalLHEScale7_deepflav_NOsnap_eval/{sample}_Histos.root"
    )
    ...
    c.GetPad(1).SetLogy()
    c.Update()
    c.SaveAs(f"LHEScale/{sample}/{histo}.pdf")
