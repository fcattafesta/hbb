...
def load_data(dirs):
    ...
    for i, file in enumerate(files):
        ...
        print(score_total.shape, score)
        print(hadronFlavour_total.shape, hadronFlavour)

    print(score_total.shape, score)
    print(hadronFlavour_total.shape, hadronFlavour)

    # concatenate the score columns in a single array
    score_total = np.concatenate(
        (
            score_total[:, 0].reshape(-1, 1),
            score_total[:, 1].reshape(-1, 1),
        ),
        axis=1,
    )
