...
parser.add_argument("-p", "--postfit",
                    help="plot postfit plot", action="store_true")
parser.add_argument("-v", "--variablesToFit", nargs="*")
parser.add_argument("-f", "--folder", default="out/", help="input folder")
parser.add_argument("-o", "--outfolder", default="figure/",
                    help="fgure output folder folder")
parser.add_argument("-w", "--workspace", default="workspace/",
                    help="workspace output folder")
parser.print_help()
...
def makeRatioMCplot(h):
    ...
    for n in range(hMC.GetNbinsX()+1):
       # hMC.SetBinError(n,  hMC.GetBinError(n)/hMC.GetBinContent(n) if hMC.GetBinContent(n)>0 else 0 )
        e = hMC.GetBinError(
            n)/hMC.GetBinContent(n) if hMC.GetBinContent(n) > 0 else 0
...
#        h.GetXaxis().SetTitle(str(h.GetName()).split("___")[0])
        ...
        h.GetXaxis().SetTitle(labelVariable[xKey] if xKey in list(
            labelVariable.keys()) else xKey)
    else:
...
def writeYields(ftxt, gr, integral, error, dataEvents):
    line = "%s,%s,%s,%s " % (gr, round(integral[gr]["nom"], 5), round(
        error[gr], 5), round(integral[gr]["nom"]/dataEvents, 5))
...
def makeAlternativeShape(hn, sy, f, nominalSample, alternativeSamples, alphaUp=+1, alphaDown=-1):
    ...
    if not altSampleDown in f:
        f[altSampleDown] = ROOT.TFile.Open(
            folder+"/%sHistos.root" % altSampleDown)
    ...
    if hn.split("___")[0] in list(model.rebin.keys()):
        histoNom = (histoNom.Rebin(len(model.rebin[hn.split("___")[
                    0]])-1, "hnew"+sy, array('d', model.rebin[hn.split("___")[0]])))
        histoUp = (histoUp.Rebin(len(model.rebin[hn.split("___")[
                   0]])-1, "hnew"+sy, array('d', model.rebin[hn.split("___")[0]])))
        histoDown = (histoDown.Rebin(len(model.rebin[hn.split("___")[
                     0]])-1, "hnew"+sy, array('d', model.rebin[hn.split("___")[0]])))
...
def makeEnvelopeShape(hn, sy, f, d, model):
    ...
    envelopeFunctionParameterValues = model.systematicDetail[
        sy_base]["envelopeFunctionParameterValues"]
...
# uncomment if you want to use the standard binning (ie. ignore "envelopeNBins")
    if hn.split("___")[0] in list(model.rebin.keys()):
        model.systematicDetail[sy_base]["envelopeBinning"][(
            hn, d)] = model.rebin[hn.split("___")[0]]
    ...
    if (hn, d) in model.systematicDetail[sy_base]["envelopeBinning"]:
        ...
        nomHistoRebinned = nomHistoRebinned.Rebin(
            len(envelopeBinning)-1, nomHistoRebinned.GetName(), array('d', envelopeBinning))
    else:
    ...
    try:
        ...
        print("WARNING makeEnvelopeShape: LHApdf_down not found",
              hn, sy, f, d, model)
    ...
    funct = ROOT.TF1("funct", envelopeFunction, nomHistoRebinned.GetXaxis(
    ).GetXmin(), nomHistoRebinned.GetXaxis().GetXmax())
#    funct = ROOT.TF1("funct",envelopeFunction,envelopeFunctionRange[0],envelopeFunctionRange[1])
    ...
    while hs and hs.GetMaximum() > 0:
        if envelopeBinning:
            hs = hs.Rebin(len(envelopeBinning)-1, nomHistoRebinned.GetName(),
                          array('d', envelopeBinning)).Clone(hs.GetName())
        if i == 0:
            if LHApdf_min == 91400:
                hs0 = hs.Clone("hs0")
                i += 1
        ...
        if abs(funct.GetParameter(0)-1) < 0.2:
            par2 += (funct.GetParameter(envelopeFunctionParameter) -
                     envelopeFunctionParameterValues[envelopeFunctionParameter])**2
        else:
            badFit += 1
            print("BAD Fit", hn, sy, f, d, model, funct.GetParameter(
                0), funct.GetParameter(1), end=' ')
        i = i + 1
        hs = f[d].Get(findSyst(hn, pdf+str(i), f[d]))

    # if not hessian
    if not ((LHApdf_min < 0 and pdf == pdfHessian) or LHApdf_min in [303000, 303200, 304200, 304400, 304600, 304800, 305800, 306000, 306200, 306400, 91400]) and (i-badFit) > 0:
        print("REPLICAS, not HESSIAN for", hn, sy, f, d, model)
        par2 = (par2/(i-badFit))

    funct.SetParameters(*envelopeFunctionParameterValues)
    if "Up" in sy:
        funct.SetParameter(envelopeFunctionParameter,
    ...
    print("Creating %s using %s" % (nhisto.GetName(), pdf),
...
#        model.systematicDetail[sy_base]["envelopeBinning"][(hn, d)] = model.rebin[hn.split("___")[0]]
    ...
    hs = hs.Rebin(len(envelopeBinning)-1, hs.GetName(),
...
#        Calculate ratio wrt to PDF0:
    ...
    while hs and hs.GetMaximum() > 0:
        ...
        for bin_ in range(1, len(ratio)):
            rat = hs.GetBinContent(bin_)/hs0.GetBinContent(
                bin_) if hs0.GetBinContent(bin_) > 0 else 0.
            sums[bin_] += rat
    ...
    for bin_ in range(len(ratio)-1):
        if sumSquares[bin_] > 0:
            ...
            # if not hessian
            if not ((LHApdf_min < 0 and pdf == pdfHessian) or LHApdf_min in [303000, 303200, 304200, 304400, 304600, 304800, 305800, 306000, 306200, 306400, 91400]):
                rms = rms*(i**0.5)
            meanrms += rms
            ngood += 1
        else:
    ...
    funct = ROOT.TF1("funct", envelopeFunction.format(up=(1. if "Up" in sy else -1.), rms=meanrms, xmin=nomHistoRebinned.GetXaxis().GetXmin(),
                                                      xmax=nomHistoRebinned.GetXaxis().GetXmax()), nomHistoRebinned.GetXaxis().GetXmin(), nomHistoRebinned.GetXaxis().GetXmax())
    nhisto = f[d].Get(hn).Clone(hn+sy)
    if hn.split("___")[0] in list(model.rebin.keys()):
        nhisto = (nhisto.Rebin(len(model.rebin[hn.split("___")[
                  0]])-1, "hnew"+sy, array('d', model.rebin[hn.split("___")[0]]))).Clone(hn+"rebinned")
    ...
    for bin_ in range(len(nhisto)-1):
        ...
        f = funct.Eval(x)
        nhisto.SetBinContent(
            bin_, copyhisto.GetBinContent(bin_) * (1. + f * rms))
...
histoNames = list(set([x.GetName() for y in list(f.keys())
                       for x in f[y].GetListOfKeys()]))
...
def fill_datasum(f, gr, samplesToPlot, SumTH1, stack, stackSys, hn, myLegend, ftxt, lumis=[], data=False):
    ...
    for n in range(len(samplesToPlot[gr])):
        ...
        if f[d]:
            ...
            if h:
                if hn.split("___")[0] in list(model.rebin.keys()):
                    # print "Rebin",hn
                    h = (h.Rebin(len(model.rebin[hn.split("___")[
                         0]])-1, "hnew", array('d', model.rebin[hn.split("___")[0]])))
                h = h.Clone(hn+"rebinned")
                if data:
                    h.SetMarkerStyle(10)
                else:
                    #if postfit : addFitVariation( h, fitVariation(model, f, d, hn, h, histoSingleSyst))
                    print(h.GetSumOfWeights(), h.GetEntries(),
                          lumi*samples[d]["xsec"], d)
                        ...
                        if not data:
                            if sy_base in model.systematicDetail and "alternativeSamples" in model.systematicDetail[sy_base]:
                                if d in model.systematicDetail[sy_base]["alternativeSamples"]:
                                    hs = makeAlternativeShape(hn, sy, f, d, model.systematicDetail[sy_base]["alternativeSamples"][
                                                              d], model.systematicDetail[sy_base]["powerUp"], model.systematicDetail[sy_base]["powerDown"])
                                else:
                                    hs = f[d].Get(hn).Clone(hn+sy)
                                    if hs and hn.split("___")[0] in list(model.rebin.keys()):
                                        hs = (hs.Rebin(len(model.rebin[hn.split("___")[0]])-1, "hnew"+sy, array(
                                            'd', model.rebin[hn.split("___")[0]]))).Clone(hs.GetName()+"rebinned")
                            ...
                            if hs:
                                ...
                                if sy_base in list(model.systematicDetail.keys()) and "normalizationType" in list(model.systematicDetail[sy_base].keys()) and model.systematicDetail[sy_base]["normalizationType"] == "shapeOnly" and hs.Integral(0, hs.GetNbinsX()+1) > 0:
                                    hs.Scale(h.Integral(0, h.GetNbinsX()+1) /
                                ...
                                addHistoInTStack(
                                    h, stackSys, all_histo_all_syst, gr, hn, sy, d, makeWorkspace)
                        else:
                            addHistoInTStack(
                                h, stackSys, all_histo_all_syst, gr, hn, sy, d, makeWorkspace)
                else:
    ...
    if not data:
        writeYields(ftxt, gr, integral, error,
...
#        os.system("git status HEAD > "+outpath+"/git_status.txt")
        ...
        histosSignal[hn] = {}
        for gr in model.data:
            h = fill_datasum(f, gr, model.data, SumTH1=datasum, stack=datastack,
                             stackSys=datasumSyst, hn=hn, myLegend=myLegend, ftxt=ftxt, data=True)
        ...
        for gr in model.backgroundSorted:
            h = fill_datasum(f, gr, model.background, SumTH1=histosum, stack=histos,
                             stackSys=histosumSyst, hn=hn, myLegend=myLegend, ftxt=ftxt, lumis=lumis)
            dictLegendBackground[gr] = h

        for gr in model.signal:
            h = fill_datasum(f, gr, model.signal, SumTH1=histoSigsum, stack=histosSig,
                             stackSys=histoSigsumSyst, hn=hn, myLegend=myLegend, ftxt=ftxt, lumis=lumis)
       ...
       # myLegend.AddEntry(None, "", "")
        ...
        if hn in datasum.keys():
            datasum[hn].SetMinimum(
                max(0.1*datasum[hn].GetMinimum(), 0.1))  # zoom out y axis
            datasum[hn].Draw("E P")
        # datastack[hn].GetXaxis().SetTitle(hn)
            ...
            histos[hn].SetMinimum(
                max(0.1*histos[hn].GetMinimum(), 0.1))  # zoom out y axis
            histos[hn].Draw("hist")
#  histos[hn].Draw("hist")
        ...
        t0 = makeText(0.65, 0.85, labelRegion[hn.split("___")[1]] if hn.split(
            "___")[1] in list(labelRegion.keys()) else hn.split("___")[1], 61)
        t1 = makeText(0.25, 0.91, "CMS", 61)
        t2 = makeText(0.32, 0.91, str(year), 42)
        t3 = makeText(0.90, 0.91, lumi % (lumitot/1000.)+"  (13 TeV)", 42)
        td = makeText(0.85, 0.78, "d = " +
                      d_value(histosum[hn], histoSigsum[hn]), 42, 0.04)
        t0.Draw()
        t1.Draw()
        t2.Draw()
        t3.Draw()
        ...
        if hn in datasum.keys():
          ...
          for n in range(datasum[hn].GetNbinsX()+2):
            if datasum[hn].GetBinContent(n) > 0:
                ratio.SetBinError(n, datasum[hn].GetBinError(n)/(histosum[hn].GetBinContent(
                    n) if histosum[hn].GetBinContent(n) > 0 else datasum[hn].GetBinContent(n)))
          ...
          tchi2 = makeText(0.22, 0.22, "#chi^{2}="+str(
            round(datasum[hn].Chi2Test(histosum[hn], "UWCHI2/NDF"), 2)), 42, 0.025)
          tKS = makeText(
            0.32, 0.22, "KS="+str(round(datasum[hn].KolmogorovTest(histosum[hn]), 2)), 42, 0.025)
          tchi2.Draw()
          tKS.Draw()
                ...
                for d in model.data[group]:
                    for syst in list(all_histo_all_syst[hn][d].keys()):
                        if not syst in all_histo_all_syst[hn]["data"+year]:
                            all_histo_all_syst[hn]["data" +
                                                   year][syst] = all_histo_all_syst[hn][d][syst].Clone()
                        else:
                            all_histo_all_syst[hn]["data" +
                                                   year][syst].Add(all_histo_all_syst[hn][d][syst])
